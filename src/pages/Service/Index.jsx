...
export default function Service() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    instance.delete(`service/${id}`)
                        .then(res => {
                            setServices(currentService => currentService.filter(service => service.id !== id));
                            Swal.fire({
                                title: "Deleted!",
    ...
    return(
        <Case>
            <div className="block m-auto bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 min-h-screen">
                ...
                <h5 className="mb-1 ml-5 text-3xl font-medium text-gray-900 dark:text-white text-shadow-blue">Service</h5>
    ...
    {services.map((service, id) => (
        <tr key={service.id} className="border-b dark:border-neutral-500">
            <td className="whitespace-nowrap px-6 py-4 text-white">{id + 1}</td>
            <td className="whitespace-nowrap px-6 py-4 text-white">{service.barang}</td>
            
            <td className="whitespace-nowrap px-6 py-4 text-white">{service.keterangan}</td>
            <td className="whitespace-nowrap px-6 py-4 text-white">{service.periksa}</td>
            ...
            <td className="whitespace-nowrap px-6 py-4">
                ...
                <button onClick={() => deleteStockStuff(service.id,  service.stuff_id)} type="button" className="px-4 py-2 bg-red-500 rounded-lg mr-2 font-bold text-white hover:bg-red-700">Hapus</button>
            </td>
        </tr>
    ))}
</tbody>
