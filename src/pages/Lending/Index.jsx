...
export default function Lending() {
        ...
        const delateLending = (id, name) => {
            Swal.fire({
                title: "Are you sure?",
                text: `Ingin membatalkan peminjaman '${name}'.`,
                icon: "warning",
                ...
                if (result.isConfirmed) {
                    instance.delete(`lending/${id}`)
                        .then(res => {
                            setLendings(currenLending => currenLending.filter(lending => lending.id !== id));
        ...
        const handleSearch = (event) => {
            ...
            const filtered = lendings.filter(lending =>
    ...
    {filteredLendings.map((lending, id) => (
        <tr key={lending.id} className="border-b dark:border-neutral-500">
            ...
            <td className="whitespace-nowrap px-6 py-4">
            {
                ...
                <button onClick={() => delateLending (lending.id, lending.name)} type="button" className="px-4 py-2 bg-green-500 rounded-lg mr-2 font-bold text-white hover:bg-green-700">Batal Peminjaman</button>
            </td>
        </tr>
    ))}
</tbody>
